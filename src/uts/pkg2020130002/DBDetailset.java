...
//                d.setArmorid(rs.getString("armor_id"));
    ...
    public ObservableList<DetailsetModel> LoadSet(String weaponid, int minset) {
        try {
            ObservableList<DetailsetModel> TableData = FXCollections.observableArrayList();
            Koneksi con = new Koneksi();
            con.bukaKoneksi();
            con.statement = con.dbKoneksi.createStatement();
            ResultSet rs
                    = con.statement.executeQuery("Select set_equip_id, set_name, d.set_id, min_set "
                            + " from detailset d join setequipment s on (d.set_id=s.set_id) where "
                            + " weapon_id = '" + weaponid + "' and min_set <= '" + minset + "'");
